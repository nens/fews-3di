...
from openapi_client import (
    ApiClient, Configuration, AuthApi,
    SimulationsApi, ThreedimodelsApi)
...
laterals = get_lateral_timeseries(
    convert_path(r'..\input\lateral.csv'), tstart, tend)
...
if set_initial_state:
    sim_api.simulations_initial_saved_state_create(
        sim_id, data=initial_state_data)
...
time_indexes = np.argwhere(
    (precipitation_datetimes >= tstart) &
    (precipitation_datetimes <= tend)).flatten().tolist()
...
sim_upload_rain = sim_api.simulations_events_rain_rasters_netcdf_create(
    sim_id, data={
        'filename': rainfilename.replace('.nc', '_{}.nc'.format(sim_id))}
)
...
while processing:
    upload_status = sim_api.simulations_events_rain_rasters_netcdf_list(
        sim_id)
...
# precipation_datetimes
time_indexes = np.argwhere(
    (evap_datetimes >= tstart) &
    (evap_datetimes <= tend)).flatten().tolist()
...
sim_upload_evap =\
        sim_api.simulations_events_sources_sinks_rasters_netcdf_create(
            sim_id, data={
                'filename': evapfilename.replace(
                    '.nc', '_{}.nc'.format(sim_id))})
...
while processing:
    upload_status =\
        sim_api.simulations_events_sources_sinks_rasters_netcdf_list(
            sim_id)
    if upload_status.results[0].file.state == 'processed':
        processing = False
    else:
...
while pending:
    ...
    if latest_sim_status.name in ['finished', ]:
        pending = False
    sleep(5)
...
def download_file(results, file_name, path):
    results = [
        x for x in results if x.filename.lower() == file_name.lower()]
    if results:
...
download_file(
    sim_results.results, 'simulation.log',
    convert_path(r'..\output\simulation.log'))

# New zipfile with all logs (combined)
download_file(
    sim_results.results, f'log_files_sim_{sim_id}.zip',
    convert_path(r'..\output\simulation.zip'))

download_file(
    sim_results.results, 'flow_summary.log',
    convert_path(r'..\output\flow_summary.log'))

download_file(
    sim_results.results, 'results_3di.nc',
...
df.to_csv(
    convert_path(r'..\output\discharges.csv'),
...
time_indexes = np.argwhere(
    (ow_datetimes >= tstart) &
    (ow_datetimes <= tend)).flatten().tolist()

# Create new file with only time_indexes
create_file_from_source(convert_path(ow_path), convert_path(r'..\\output\\ow.nc'), time_indexes)
